...
window.AdditionAssociativityLeft = function(svg, callback, cancelCallback, getIsCanceled, setIsCanceled) {
...
svg.attr("width", maxWidth)
  ...
  .attr("viewBox", `0 0 800 500`);
    ...
    const delayConsolidateSum = 2000; // delay for ex and its brace
    const durationConsolidateSum = 4000; // duration for ex and its brace

    const delayToSigns = delayConsolidateSum + durationConsolidateSum + 2000;
    const durationSigns = 1000;


    const totalDuration = delayToSigns + durationSigns + 500;
...
if (window.innerWidth < 600) {
    ...
    const midPointLeft = [startPoint[0] + (endPoint[0] - startPoint[0]) * 2 / 5, startPoint[1]];

    // Add the top left brace (e) with delay and duration
    const midPointRight = [startPoint[0] + (endPoint[0] - startPoint[0]) * 4 / 5, startPoint[1]];



    // Add the top right brace (e') with delay and duration
    const topRightBrace = svg.append("path")
        .attr("d", makeCurlyBrace(midPointRight[0], midPointRight[1] - 20, endPoint[0], endPoint[1] - 20, 20, 0.5, true))
        .attr("fill", "none")
        .attr("stroke", "gray")
        .attr("stroke-width", brace_stroke_width)
        .style("opacity", 1);
    ...
    const topRightLabel =svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (midPointRight[0] + endPoint[0]) / 2+5)
        ...
        .style("opacity", 1)
        .transition()
        .delay(delayConsolidateSum)
        .duration(durationConsolidateSum)
        .style("opacity", 1);


    const topUmbrellaLeftBrace = svg.append("path")
        .attr("d", makeCurlyBrace(startPoint[0], startPoint[1] - 20, midPointRight[0], midPointRight[1] - 20, 20, 0.5, true))
        .attr("fill", "none")
        .attr("stroke", "gray")
        .attr("stroke-width", brace_stroke_width)
        .style("opacity", 1)
    ...
    svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (startPoint[0] + midPointRight[0]) / 2)
    ...
    const topLeftBrace = svg.append("path")
        .attr("d", makeCurlyBrace(startPoint[0], startPoint[1] - 20, midPointLeft[0], midPointLeft[1] - 20, 20, 0.5, true))
        .attr("fill", "none")
        .attr("stroke", "gray")
        .attr("stroke-width", brace_stroke_width)
        .style("opacity", 0)
    ...
    const topLeftLabel = svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (startPoint[0] + midPointLeft[0]) / 2)
        .attr("y", startPoint[1] - 60)  // Adjusted y-position for larger font
        .attr("text-anchor", "middle")
        .attr("fill", "gray")
        .attr("font-size", fontSize)  // Set font size
    ...
    // Add the top right brace (e') with delay and duration
    const topMiddleBrace = svg.append("path")
        .attr("d", makeCurlyBrace(midPointLeft[0], midPointLeft[1] - 20, midPointRight[0], midPointRight[1] - 20, 20, 0.5, true))
        .attr("fill", "none")
        .attr("stroke", "gray")
        .attr("stroke-width", brace_stroke_width)
        .style("opacity", 0)
        ...
        .style("opacity", 1);
            ...
            // Add the top right label (e') with delay and duration
    const topMiddleLabel = svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (midPointLeft[0] + midPointRight[0]) / 2)
        .attr("y", endPoint[1] - 60)  // Adjusted y-position for larger font
        .attr("text-anchor", "middle")
        .attr("fill", "gray")
        .attr("font-size", fontSize)  // Set font size
...
// Add "+" label between topRightLabel and topMiddleLabel
const rightPlusLabel = svg.append("text")
    .attr("class", "label-e-father")
    .attr("x",  (midPointLeft[0] + midPointRight[0] + midPointRight[0] + endPoint[0]) / 4)
    .attr("y", endPoint[1] - 60)  // Same y-position as the other labels
    .attr("text-anchor", "middle")
    .attr("fill", "gray")
    ...
    .transition()
    .delay(delayToSigns)
    .duration(durationSigns)
    .style("opacity", 1);

// Add "+" label between topMiddleLabel and topLeftLabel
const leftPlusLabel = svg.append("text")
    .attr("class", "label-e-father")
    .attr("x", midPointLeft[0])
    .attr("y", endPoint[1] - 60)  // Same y-position as the other labels
    .attr("text-anchor", "middle")
    .attr("fill", "gray")
    .attr("font-size", fontSize)
