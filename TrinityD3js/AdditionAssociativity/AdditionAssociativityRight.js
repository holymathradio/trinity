...
window.AdditionAssociativityRight = function(svg, callback, cancelCallback, getIsCanceled, setIsCanceled) {
...
svg.attr("width", maxWidth)
  ...
  .attr("viewBox", `0 0 800 500`);
    ...
    const delayToConsolidateSum = 4000;

    const delayConsolidateSum = delayToOperands  + durationOperands  + delayToConsolidateSum; // delay for ex and its brace
    const durationConsolidateSum = 4000; // duration for ex and its brace

    const totalDuration = delayConsolidateSum + durationConsolidateSum + 500;
...
;
    ...
    const topUmbrellaRightBrace = svg.append("path")
        .attr("d", makeCurlyBrace(midPointLeft[0], midPointLeft[1] - 20, endPoint[0], endPoint[1] - 20, 20, 0.5, true))
        .attr("fill", "none")
        .attr("stroke", "gray")
        .attr("stroke-width", brace_stroke_width)
        .style("opacity", 1)
    ...
    svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (midPointLeft[0] + endPoint[0]) / 2)
