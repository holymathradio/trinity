console.log("MultiplicationDistributivityUpAndDown.js loaded");

window.MultiplicationDistributivityUpAndDown = function(svg, callback, cancelCallback, getIsCanceled, setIsCanceled) {
    console.log("MultiplicationDistributivityUpAndDown function called");
    ...
    const delayGoingUp= delayTime + durationTime ;
    const durationGoingUp = 3000;

    const delayGoingDown = delayGoingUp + durationGoingUp + 4000;
    const durationGoingDown = 3000;

    const delayGoingUpAgain = delayGoingDown + durationGoingDown + 3000;
    const durationGoingUpAgain  = 3000;

    const totalDuration = delayGoingUpAgain + durationGoingUpAgain + 500;
    ...
    function drawMiddleRectangles(svg, axes, N, y) {
        const rectWidth = 5;
        const rectHeight = 10;

        svg.selectAll(".middle-rect").remove();

        for (let i = 1; i < N; i++) {
            const x = i * (4 / N);
            const [xPos, yPos] = axes.c2p(x, y);
...
function drawMiddleRectangles(svg, axes, N, leftPoint, rightPoint, shouldShowRectangles) {
    const rectWidth = 5;
    const rectHeight = 20;
...
function createMiddleRectangles(svg, axes, N) {
    ...
    for (let i = 0; i <= N; i++) {
        ...
        svg.append("rect")
            .attr("class", i === 0 ? "middle-rect left-extremity" : 
                  i === N ? "middle-rect right-extremity" : "middle-rect")
            .attr("x", xPos)
            .attr("y", yPos)
            .attr("width", rectWidth)
            .attr("height", rectHeight)
...
function updateMiddleRectangles(svg, axes, N, leftPoint, rightPoint, shouldShowRectangles) {
    const middleRects = svg.selectAll(".middle-rect");
    
    if (shouldShowRectangles) {
        middleRects.style("display", "block");
        
        // Update left extremity rectangle
        svg.select(".middle-rect.left-extremity")
            .attr("x", leftPoint[0])
            .attr("y", leftPoint[1]);
        ...
        for (let i = 1; i < N; i++) {
            ...
            svg.selectAll(".middle-rect:not(.left-extremity):not(.right-extremity)")
        ...
        svg.select(".middle-rect.right-extremity")
            .attr("x", rightPoint.x)
            .attr("y", rightPoint.y);
    } else {
...
const timeoutIdUp = 
    setTimeout(() => {
moveSegmentWithDelay(svg, axes, N, true,durationGoingUp, endPrior=false);
}, delayGoingUp); // 5000 ms delay as an example


// After some delay, move downwards
const timeoutIdDown = setTimeout(() => {
    moveSegmentWithDelay(svg, axes, N, false,durationGoingDown, endPrior=false);
}, delayGoingDown); // 5000 ms delay as an example


// After some delay, move downwards
const timeoutIdUpAgain = setTimeout(() => {
    moveSegmentWithDelay(svg, axes, N, true,durationGoingUpAgain, endPrior=true);
}, delayGoingUpAgain); // 5000 ms delay as an example
