console.log("MultiplicationInverseIntro.js loaded");
window.MultiplicationInverseIntro = function(svg, callback, cancelCallback, getIsCanceled, setIsCanceled) {
    console.log("MultiplicationInverseIntro function called");
...
svg.attr("width", maxWidth)
  ...
  .attr("viewBox", `0 0 800 500`);
    ...
    const delayTick= delayTime + durationTime + 500 ;
    const durationTick = 1000;



    const totalDuration = delayTick + durationTick + 500;
...
// Append the initial path to the SVG
const transitioningPathRightBis = svg.append("path")
    .attr("d", createArc(topPoint, axes.c2p(1,0 ),-Math.PI * 1.383,arrowGap = 15)) //parameters from previous scene
    .attr("fill", "none")
    .attr("stroke", "yellow")
    .attr("stroke-width", 2)
