console.log("MultiplicationInverseWide.js loaded");

window.MultiplicationInverseWide = function(svg, callback, cancelCallback, getIsCanceled, setIsCanceled) {
    console.log("MultiplicationInverseWide function called");
...
svg.attr("width", maxWidth)
  ...
  .attr("viewBox", `0 0 800 500`);
    ...
    const totalDuration = delayTime + durationTime + 100;
...
function calculateArcParameters(startDrawPoint, endDrawPoint, arcAngleRadians = Math.PI / 3) {
    ...
    return {
        ...
        endAngle: angle + Math.PI + arcAngleRadians + arcAngleRadians /1.1,
...
/* explore angles 
// Define different angleHeight values (in radians)
const angleHeights = [
    ...
    -Math.PI /1.1,
    -Math.PI /1.05,
    -Math.PI * 1.35, // this reproduces the original curve
        -Math.PI *2,
        -Math.PI / 2,   // -90 degrees
    -Math.PI / 3,   // -60 degrees
    -Math.PI / 6,   // -30 degrees
...
// Create arcs with different angleHeights
angleHeights.forEach((angleHeight, index) => {
    svg.append('path')
        .attr('d', createArc(topPoint, endPoint, angleHeight))
        .attr('fill', 'none')
...
svg.selectAll('text')
    ...
    .attr('x', 520)
    .attr('y', (d, i) => 20 + i * 20)
    .text((d, i) => `${Math.round(d * 180 / Math.PI)}°`)
...
const transitioningPathLeft = svg.append("path")
    .attr("d", createArc(topPoint, axes.c2p(1,0 ),-Math.PI * 1.38,arrowGap = 15))
    .attr("fill", "none")
    .attr("stroke", "yellow")
    .attr("stroke-width", 2)
    ...
    transitioningPathRight.transition()
        ...
        .attrTween("d", function() {
            return function(t) {
                ...
                angle_right = -Math.PI * 1.38*(1+t*0.5)
