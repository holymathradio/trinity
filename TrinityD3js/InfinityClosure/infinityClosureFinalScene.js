console.log("infinityClosureFinalScene.js loaded");
window.infinityClosureFinalScene = function(svg, callback, cancelCallback, getIsCanceled, setIsCanceled) {
    console.log("infinityClosureFinalScene function called");
...
if (window.innerWidth < 600) {
    ...
    let shiftLabelArrow = axes.c2p(0,0.8)[1]-axes.c2p(0,0)[1];


    let shiftLabelPrinciple = (axes.c2p(0,1.1)[1]-axes.c2p(0,0)[1]);
    ...
    function makeCurlyBrace(x1, y1, x2, y2, w, q, isTop = false,mobileRatio = 1) {
        ...
        var smallFeetDownwardPath = `l 0 ${-directionFactor * 3 }`; // add a longer tick in the feets
...
//update
    ...
    const topLabelPriorArrow = svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (startPoint_e[0] + endPoint_eprime_aligned[0]) / 2)
        .attr("y", startPoint_e[1]+ shiftLabelArrow)  // Adjusted y-position for larger font
        .attr("text-anchor", "middle")
        .attr("fill", "gray")
        .attr("font-size", fontSize)  // Set font size 0.8
        .html("→");



    // Add the top right label (e')
    const bottomBraceLabel_Principle = svg.append("text")
        .attr("class", "label-e-father")
        .attr("x", (startPoint_e[0] + endPoint_eprime_aligned[0]) / 2)
